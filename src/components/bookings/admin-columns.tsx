...
export const adminColumns = (): ColumnDef<BookingOrder>[] => [
  {
    accessorKey: "_id",
    header: ({ column }) => (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const customerPhone = row.getValue("customer_phone") as string;
      return (
        <div className="flex items-center space-x-2">
          <User className="h-4 w-4 text-gray-400" />
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const artisanFullName = row.getValue("artisanFullName") as string;
      return (
        <div className="flex items-center space-x-2">
          <User className="h-4 w-4 text-gray-400" />
          <span className="font-medium">{artisanFullName}</span>
        </div>
      );
    },
  },
  {
    accessorKey: "artisanPhone",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Artisan Phone" />
    ),
    cell: ({ row }) => {
      const artisanPhone = row.getValue("artisanPhone") as string;
      return (
        <div className="flex items-center space-x-2">
          <User className="h-4 w-4 text-gray-400" />
          <span className="font-medium">{artisanPhone}</span>
        </div>
      );
    },
  },
